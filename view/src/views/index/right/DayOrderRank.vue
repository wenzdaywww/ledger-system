<template>
  <div class="rank-box">
    <!--  日订单量排行榜-->
    <div class="schart" id="day-order-rank"></div>
  </div>
</template>
...
export default {
  ...
  components: {},
  mounted() {
    let initChart = init(document.getElementById("day-order-rank"));
  ...
  setup() {
    ...
    //报表标题
    const rankTitle = "{0} 日订单量排行榜";
    //报表标题
    const noOrderTitle = "没有日订单量数据";
    //日订单量数据
    ...
    //日销售排行榜
    const findDayOrderRank = () => {
      axios.$http.get(request.dayOrderRank,query).then(function (res) {
        let dayOrderRank = getInstanceByDom(document.getElementById("day-order-rank"));
        let option = {
          title: {text:""},
          legend: {data:[]},
          yAxis: {data:[]},
        ...
        if (res.data && res.data.yaxis){
          ...
          let oldOption = dayOrderRank.getOption();
          for (let i = 0; i < oldOption.series.length; ++i) {
            option.series.push({name:"",data:[]});
          }
        }
        dayOrderRank.setOption(option);
      }).catch(err => {});
    }
    return {dayOrderOption,
      changeShopId
    };
  },
};
