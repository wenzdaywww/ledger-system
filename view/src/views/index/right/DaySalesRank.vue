<template>
  <div class="rank-box">
    <!--  日销售额排行榜-->
    <div class="schart" id="day-sales-rank"></div>
  </div>
</template>
...
export default {
  ...
  components: {},
  mounted() {
    let initChart = init(document.getElementById("day-sales-rank"));
  ...
  setup() {
    ...
    //报表标题
    const rankTitle = "{0} 日销售额排行榜";
    //报表标题
    const noOrderTitle = "没有日销售额数据";
    //日销售额数据
    ...
    //日销售排行榜
    const findDaySalesRank = () => {
      axios.$http.get(request.daySalesRank,query).then(function (res) {
        let daySalesRank = getInstanceByDom(document.getElementById("day-sales-rank"));
        let option = {
          title: {text:""},
          legend: {data:[]},
          yAxis: {data:[]},
        ...
        if (res.data && res.data.yaxis){
          ...
          let oldOption = daySalesRank.getOption();
          for (let i = 0; i < oldOption.series.length; ++i) {
            option.series.push({name:"",data:[]});
          }
        }
        daySalesRank.setOption(option);
      }).catch(err => {});
    }
    return {daySalesOption,
      changeShopId
    };
  },
};
